...
import { getTargets, setTargets, getSuggestedTargets } from '../api/client';
...
// Only used if GET /api/targets/suggested is unreachable, or before that first
// call resolves — the suggested endpoint is now the actual source of truth for
// preset numbers (per fix #8), these are just an offline safety net.
const FALLBACK_DEFAULTS: Record<GoalType, Goals> = {
...
const GOAL_TYPE_KEY = 'calorific_goal_type';

function loadGoalType(): GoalType {
    const saved = localStorage.getItem(GOAL_TYPE_KEY);
    return saved === 'lose' || saved === 'maintain' || saved === 'gain' ? saved : 'maintain';
}
...
export function loadGoals(): Goals {
    ...
    return FALLBACK_DEFAULTS.maintain;
}

function GoalsPage() {
    const navigate = useNavigate();
    const [goalType, setGoalType] = useState<GoalType>(loadGoalType);
    const [goals, setGoals] = useState<Goals>(FALLBACK_DEFAULTS[loadGoalType()]);
    ...
    async function applyPreset(type: GoalType) {
        ...
        setSaved(false);
        setError('');
        localStorage.setItem(GOAL_TYPE_KEY, type);
        setSuggesting(true);
        try {
            const suggested = await getSuggestedTargets(type);
            setGoals({
            ...
            setError(err.message || "Couldn't fetch suggested targets — showing a rough estimate instead. You can still edit the numbers below.");
            setGoals(FALLBACK_DEFAULTS[type]);
        } finally {
            setSuggesting(false);
        }
    }
            ...
            <div style={styles.card}>
                ...
                <div style={styles.goalRow}>
                    {(['lose', 'maintain', 'gain'] as GoalType[]).map(type => (
                        <button key={type} disabled={suggesting}
                            style={{ ...(goalType === type ? styles.goalBtnActive : styles.goalBtn), opacity: suggesting ? 0.6 : 1 }}
                ...
                {suggesting && <p style={{ fontSize: 12, color: '#188159', margin: '10px 0 0' }}>Calculating your suggested targets...</p>}
            </div>
