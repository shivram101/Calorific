...
function calculateSuggestedTargets(profile) {
  ...
  const goalAdjustments = {
    lose:     -500,  // aggressive cut
    maintain:    0,  // maintenance
    build:     200,  // lean bulk for muscle gain
    gain:      400,  // standard bulk
  };

  // Macro splits vary by goal — build muscle prioritises protein
  const macroSplits = {
    lose:     { protein: 0.30, carbs: 0.40, fat: 0.30 }, // higher protein to preserve muscle on cut
    maintain: { protein: 0.25, carbs: 0.45, fat: 0.30 },
    build:    { protein: 0.35, carbs: 0.40, fat: 0.25 }, // high protein for muscle synthesis
    gain:     { protein: 0.25, carbs: 0.50, fat: 0.25 }, // carb-heavy for mass
  };
  ...
  const proteinTarget = Math.round((calorieTarget * split.protein) / 4); // 4 cal/g
  const carbTarget    = Math.round((calorieTarget * split.carbs)   / 4); // 4 cal/g
  const fatTarget     = Math.round((calorieTarget * split.fat)     / 9); // 9 cal/g
